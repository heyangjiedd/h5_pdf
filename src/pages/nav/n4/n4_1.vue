<template>
  <div class="page">
    <HeaderBar :back="false" :isShow="!swiperC" :isFixed="false" right="1/2">努力成长的我</HeaderBar>
    <div class="content" v-for="(item,index) in data" :key="index">
      <p class="text">{{item.content}}</p>
      <img class="img" :key="index" :src="getBase64Img(item.url,`n4_1${index}`)" :id="`n4_1${index}`" alt="">
    </div>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    ...mapActions('user',['getUserInfo','getCYM']),
    getData(){
      if(this.user){
        this.userInfo = this.user;
        this.getCYM({studentId:this.$route.query.id,year:this.year}).then(res => {
          if(res.code == 1){
